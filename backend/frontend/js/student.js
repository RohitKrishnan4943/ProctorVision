...
async function validateAndStartExam(examCode) {
    try {
        ...
        // Clean the exam code (remove spaces, convert to uppercase)
        const cleanExamCode = examCode.trim().toUpperCase();
        
        // Try backend first
        try {
            const response = await fetch(
                `${window.API_BASE_URL}/exams/code/${cleanExamCode}`,
                {
                    headers: {
            ...
            if (response.ok) {
                ...
                console.log("✅ Exam found in backend:", exam);
                
                // Store exam ID for later use
                currentExamId = exam.id;
                currentExamCode = cleanExamCode;
                ...
                console.log("Exam not found in backend, checking localStorage...");
                // Continue to localStorage check below
            }
        } catch (backendError) {
            console.warn("Backend not available, checking localStorage:", backendError);
        }
        
        // Fallback to localStorage
        console.log("Checking localStorage for exam code:", cleanExamCode);
        const exams = JSON.parse(localStorage.getItem('exams')) || [];
        
        // Find exam by code (case-insensitive)
        const exam = exams.find(e => e.examCode && e.examCode.toUpperCase() === cleanExamCode);
        
        if (!exam) {
            console.error("❌ Exam not found. Available exams:", exams.map(e => e.examCode));
            showAccessModal('Exam Not Found', 
                `No exam found with code: ${cleanExamCode}\n\nPlease check the code and try again.`, 
                'error');
            return;
        }
        
        console.log("✅ Exam found in localStorage:", exam);
        ...
        if (!exam.isActive) {
            showAccessModal('Exam Inactive', 
                'This exam is no longer active. Please contact your teacher.', 
                'error');
            return;
        }
        ...
        if (exam.accessType === 'specific') {
            if (!exam.allowedStudents || !exam.allowedStudents.includes(currentUser.id)) {
                showAccessModal('Access Denied', 
        ...
        if (inProgressAttempt) {
            ...
            if (continueExam) {
                ...
                window.location.href = `exam.html?code=${cleanExamCode}`;
                return;
            }
        }
        ...
        showAccessModal('Error', 
            'An error occurred while validating the exam code. Please try again.', 
            'error');
    }
}
...
async function startExamWithCode(examId) {
    ...
    try {
        console.log("🚀 Starting exam with ID:", examId, "Exam Code:", currentExamCode);
        
        // Try backend first
        try {
            const response = await fetch(`${window.API_BASE_URL}/submissions/`, {
                method: 'POST',
                headers: {
            ...
            console.log("✅ Exam started via backend:", data);

            // Save submission id
            localStorage.setItem('currentSubmission', data.id);

            closeModal('examAccessModal');

            // Redirect using EXAM CODE
        ...
        if (examIndex !== -1) {
            if (!exams[examIndex].studentsWithAccess) {
                exams[examIndex].studentsWithAccess = [];
            }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add enter key support for exam code input
...
function getAccessibleExamsForStudent(studentId, exams) {
    return exams.filter(exam => {
        ...
        if (exam.accessType === 'link') {
            ...
            return exam.allowedStudents && exam.allowedStudents.includes(studentId);
        }
        return false;
    });
}
...
function validateExamCode() {
    ...
    if (!examCode || examCode.length === 0) {
        showAccessModal('Invalid Code', 'Please enter an exam code.', 'error');
        return;
    }
...
function showExamInstructions(exam) {
    console.log("Showing instructions for exam:", exam);
    ...
    const instructions = `
        ...
        <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; margin: 20px 0;">
            ...
            <ul style="margin-left: 20px; margin-top: 10px;">
                ...
                <li><strong>Questions:</strong> ${exam.questions ? exam.questions.length : 0}</li>
                <li><strong>Total Marks:</strong> ${exam.questions ? calculateTotalMarks(exam.questions) : 'N/A'}</li>
                <li><strong>Teacher:</strong> ${exam.teacherName || exam.teacher_id || 'Unknown'}</li>
            </ul>
        </div>
        ...
        <div class="text-center" style="margin-top: 30px;">
            <button class="btn btn-primary" onclick="startExamWithCode('${currentExamId}')" style="min-width: 150px;">
...
function continueExam(examId) {
    ...
    if (currentAttempt) {
        ...
        if (exam && exam.examCode) {
            window.location.href = `exam.html?code=${exam.examCode}`;
        } else {
            alert('Exam not found. Please enter the exam code again.');
        }
    } else {
...
window.validateAndStartExam = validateAndStartExam;
window.startExamWithCode = startExamWithCode;